<template>
  <div class="auth-wrapper">
    <svg class="prodigy" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1567.45 1495.3125">
    ...
    <form @submit.prevent="handleLogin" class="form-panel">
      <input v-model="email" type="email" placeholder="Email" required />
      <input v-model="password" type="password" placeholder="Password" required />
      <div class="form-buttons">
        <button type="submit">🔐 Log In</button>
      </div>
      <p class="switch-link" @click="$emit('switch')">Register</p>
    </form>
  </div>
</template>
